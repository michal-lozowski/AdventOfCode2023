...
fs.readFile("realinput.txt", 'utf8', (err, data) => {
    ...
    function determineHandType(hand) {
        ...
        if (labels[0].includes("J")) {
            let jokerPosition = labels[0].indexOf("J")
            let jokerAmount = labels[1][jokerPosition]
            if (jokerAmount < 5) {
                labels[1].splice(jokerPosition, 1)
                labels[1][labels[1].indexOf(Math.max(...labels[1]))] += jokerAmount
            }
        }
